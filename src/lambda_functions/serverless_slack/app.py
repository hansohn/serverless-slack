...
def cloudwatch_handler(record):
    ...
    attachment = {
        ...
        "fallback": "",
        "fields": [
            {"title": "Alarm Name", "value": sns_message["AlarmName"], "short": True},
            {
                "title": "Alarm Description",
                "value": sns_message["AlarmDescription"],
                "short": False,
            },
            {"title": "Alarm Reason", "value": sns_message["NewStateReason"], "short": False},
            {"title": "Old State", "value": sns_message["OldStateValue"], "short": True},
            {"title": "Current State", "value": sns_message["NewStateValue"], "short": True},
            {
                "title": "Link to Alarm",
                "value": f"https://console.aws.amazon.com/cloudwatch/home?region={aws_region}#alarm:alarmFilter=ANY;name={parse.quote(sns_message['AlarmName'])}",
                "short": False,
            },
        ],
...
def dms_handler(record):
    ...
    try:
        if dms_event_source == "replication-instance":
            ...
            logger.info("No defined logic for dms event source '%s'.", dms_event_source)
            return attachment
    except KeyError:
        logger.info(
            "No defined logic for dms event source '%s' id '%s'.", dms_event_source, dms_event_id
        )
        return attachment
    ...
    attachment = {
        ...
        "fields": [
            {"title": "Message", "value": sns_message["Event Message"], "short": False},
            {"title": "Alarm State", "value": event_meta["severity"], "short": True},
            {
                "title": "Event",
...
def emr_handler(record):
    ...
    try:
        ...
        logger.error("Failed to decode sns message: %s", record["Sns"]["Message"])
        return attachment
    ...
    attachment = {
        ...
        "fields": [
            {"title": "Message", "value": sns_message["detail"]["message"], "short": False},
            {"title": "Alarm State", "value": sns_message["detail"]["severity"], "short": True},
            {"title": "Event", "value": sns_message["detail-type"], "short": True},
            {"title": "Source Type", "value": emr_source_type, "short": True},
            {"title": "Cluster ID", "value": sns_message["detail"]["clusterId"], "short": True},
            {"title": "Account", "value": aws_account, "short": True},
            {"title": "Region", "value": aws_region, "short": True},
        ],
...
def generic_handler(record, title="AWS Notification"):
    ...
    attachment = {
        ...
        "title": title,
        "fields": [
            {"title": "Message", "value": sns_message, "short": False},
            {"title": "Account", "value": aws_account, "short": True},
            {"title": "Region", "value": aws_region, "short": True},
        ],
        "ts": format_timestamp(sns_timestamp),
...
def post_message(attachments):
    ...
    try:
        ...
        logger.error("Fallback environment variable not found: %s", e)
        raise

    client = WebClient(token=slack_bot_token)

    try:
        response = client.chat_postMessage(channel=slack_channel, attachments=attachments)
    except SlackApiError as e:
        assert e.response["ok"] is False
        assert e.response["error"]
        logger.error("Error connecting to Slack API: %s", e.response["error"])
        raise
    ...
    if response.status_code < 400:
        ...
        logger.error(
            "Error posting message to Slack API: %s - %s",
...
def lambda_handler(event, context):
    ...
    if record["EventSource"] == "aws:sns":
        ...
        logger.error("Unsupported EventSource format: %s", record["EventSource"])
